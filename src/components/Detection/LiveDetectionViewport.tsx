...
import { X, Video, ShieldCheck } from 'lucide-react';
...
export const LiveDetectionViewport: React.FC<LiveDetectionViewportProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 3. Render FOV Circle if enabled
      ...
      if (config.showInfoEnabled) {
        ...
        ctx.fillRect(8, 8, 195, 68);
        ctx.strokeStyle = 'rgba(26, 35, 54, 0.8)';
        ctx.strokeRect(8, 8, 195, 68);

        ctx.fillStyle = '#22c55e';
        ctx.font = 'bold 11px monospace';
        ctx.fillText(`RAVEN TENSORRT ACTIVE`, 16, 23);

        ctx.fillStyle = '#cbd5e1';
        ctx.font = '10px monospace';
        ctx.fillText(`FPS: ${frameResult.fps} | Latency: ${frameResult.latencyMs}ms`, 16, 38);
        ctx.fillText(`FOV: ${config.fov}px | Bone: ${config.targetBone}`, 16, 52);
        ctx.fillText(`Entities Detected: ${frameResult.entities.length}`, 16, 65);
      }
  ...
  return (
    ...
    >
      <div className="w-[680px] h-[510px] bg-[#07090e] border border-[#0091ff]/40 rounded-2xl flex flex-col overflow-hidden shadow-[0_0_35px_rgba(0,145,255,0.25)]">
        {/* Top Header Bar */}
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#0a0d15] border-b border-[#161c2b] select-none">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-[10px] font-mono">
              <span className="px-2 py-0.5 rounded bg-[#131926] text-[#22c55e] border border-[#22c55e]/30">
                {stats.fps} FPS
              </span>
